	...
	$jx(document).ready(function() {
		...
		function Timeline(el) {
			...
			this.el.find('.timeline-event').each(function() {
				...
				$jx(this).prepend('<div class="timeline-event-node" id="timeline-event-node-'+nodeId+'"></div>');
			...
			this.eventNodeWidth = $jx(this.eventNodes[0]).outerWidth();
		...
		Timeline.prototype.showEvent = function(index) {
			...
			this.el.find('.selected-title').removeClass('selected-title');

			this.events[index].title.addClass('selected-title');
		...
		$jx('.timeline-wrap').each(function() {

			var timeline = new Timeline($jx(this));
		...
		$jx('.timeline-title').on('click', function() {

			var timeline = $jx(this).closest('.timeline-wrap').attr('id').replace('timeline-', '');

			timelines[timeline].showEvent($jx(this).closest('.timeline-event').attr('id').replace('timeline-event-', ''));

		});

		$jx('.timeline-event-node').on('click', function() {

			var timeline = $jx(this).closest('.timeline-wrap').attr('id').replace('timeline-', '');

			timelines[timeline].showEvent($jx(this).attr('id').replace('timeline-event-node-', ''));
		...
		(function() {
			...
			var targets = $jx('.'+draggableClassName);
			...
			targets.on('mousedown.nickys-draggable', function() {
				...
				targetTimeline = timelines[$jx(this).closest('.timeline-wrap').attr('id').replace('timeline-', '')];

				maxLeft = target.closest('.timeline-wrap').innerWidth();

				

				if ($jx('.timeline-title').first().css('display') != 'none') {
			...
			$jx(document).on('mousemove.nickys-draggable', function(e) {
			...
			$jx(document).on('mouseup.nickys-draggable', function() {
			...
			$jx('.timeline-draggable').get(0).addEventListener('touchstart', function(e) {
				...
				if ((x > $jx('.timeline-body').offset().left && x < $jx('.timeline-body').offset().left + $jx('.timeline-body').width()) &&

				(y > $jx('.timeline-body').offset().top && y < $jx('.timeline-body').offset().top + $jx('.timeline-body').height())) {
